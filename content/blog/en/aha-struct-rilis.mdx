...
## New in 1.4.4: Type Annotations & Inference

`let x: int = 5` — variable declarations can carry an explicit type annotation:
...
Values are type-checked at compile time — `let x: int = "hi"` is a compile error. Variables without annotations infer their type from the expression. Functions also infer their return type: `fn greet() { "hello" }` returns a String, so `len(greet())` works.

## Stabilize First, Then Move Step by Step

With F1 (structs) and F2 (type annotations) stable, the roadmap continues to **F3 — Generics**. Resource lifetimes (F5) remain frozen until the foundation is stable — per the principle *stabilize first, then move step by step*.

Follow the development on the [GitHub repo](https://github.com/qwetls/aha-lang) and check the compiler-verified [Syntax Reference](/en/syntax).
