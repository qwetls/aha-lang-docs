...
The language foundation is complete and verified by **384 automated tests** in CI:
...
### Struct (F1) — Nearly Complete

Structs now work fully at runtime:
...
Smarter inference and explicit type annotations — `let x: int = 5`. Full control for developers who want it.

### 2. Generics / Parametric Types (F3)

Write once, work for many types: generic functions and type-safe collections.
...
### 4. Resource Lifetimes (F5) — Temporarily Frozen
...
- **Branch workflow**: all development happens on the `development` branch; `main` only receives PRs with green CI.
