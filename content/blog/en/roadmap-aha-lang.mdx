...
The language foundation is complete and verified by **440 automated tests** in CI:
...
- **`List<T>` (F3e)**: generic heap-allocated dynamic arrays — `list_new`, `list_new_string`, `list_push`, `list_get`, `list_len`, `list_free`, index read/write `xs[i]`. Generic functions bind `T` from the `List<T>` hint (`fn first<T>(xs: List<T>) -> T { xs[0] }`), instantiating `first_Int`/`first_String` per call site. **Stable on `experimental/list` — 440 tests green.**
...
### 2. Map<K,V> (F3e remainder)

After `List<T>` stabilizes and merges into `development`, the next generic data structure is `Map<K,V>`.

### 3. Resource Lifetimes (F5) — Temporarily Frozen
