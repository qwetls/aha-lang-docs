...
import { getCourseSource, locales } from "@/lib/source";
...
export default async function CoursePage(props: PageProps) {
  ...
  const t = getUI(lang);
  const lessons = getCourseSource(lang).getPages();
    ...
    <OverviewContainer>
      <div className="max-w-[720px] px-6 pt-14 pb-12 space-y-10 overflow-hidden text-[#1c1917] dark:text-white/70">
        <div>
          ...
          <p className="text-[#78716c] dark:text-white/45 text-[15px]">{t.courseDescription}</p>
        </div>
