...
export default async function BlogPage(props: PageProps) {
  ...
  const t = getUI(lang);
  const posts = getBlogSource(lang)
    .getPages()
    ...
    <OverviewContainer>
      <div className="max-w-[720px] px-6 pt-14 pb-12 space-y-10 overflow-hidden text-[#1c1917] dark:text-white/70">
        <div>
          ...
          <p className="text-[#78716c] dark:text-white/45 text-[15px]">{t.blogDescription}</p>
        </div>
              ...
              <div className="flex items-center justify-between gap-4 mb-1.5">
                ...
                {post.data.date && (
                  <span className="shrink-0 text-[12px] text-[#a8a29e] dark:text-white/35">
                    {new Date(post.data.date).toLocaleDateString(t.localeDate, {
                      day: "numeric",
                      month: "long",
                      year: "numeric",
                    })}
